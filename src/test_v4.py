...
STAGES = [
    ...
    # 4: typing ls / (fresh VM: open the terminal first)
    ('05_terminal_ls', 0.0, lambda vm: (vm.move_to(959, 1038), vm.click(),
                                        time.sleep(1.2), vm.move_to(700, 300),
                                        [vm.key(k) for k in 'ls'],
                                        vm.key('spc'), vm.key('slash'),
                                        vm.key('ret'), time.sleep(1.2),
                                        vm.shot('05_terminal_ls'))),
    # 5: typing echo _aquaos (fresh VM: open the terminal first)
    ('06_terminal_echo', 0.0, lambda vm: (vm.move_to(959, 1038), vm.click(),
                                          time.sleep(1.2),
                                          [vm.key(k) for k in 'echo'],
    ...
    ('08_restored', 0.0, lambda vm: (vm.move_to(959, 1038), vm.click(),
                                     time.sleep(1.2),
                                     vm.move_to(577, 79), vm.click(),
                                     time.sleep(0.9), vm.shot('07_minimized'),
                                     vm.move_to(959, 1038), vm.click(),
                                     time.sleep(1.0), vm.shot('08_restored'))),
    # 7: green zoom, green unzoom, red close
    ('11_closed', 0.0, lambda vm: (vm.move_to(597, 79), vm.click(),
                                   time.sleep(1.0), vm.shot('09_zoomed'),
                                   vm.click(), time.sleep(1.0),
                                   vm.shot('10_unzoomed'),
                                   vm.move_to(537, 79), vm.click(),
                                   time.sleep(1.0), vm.shot('11_closed'))),
    # 8: menu open + About click closes
    ('13_menu_closed', 0.0, lambda vm: (vm.move_to(80, 20), vm.click(),
                                        time.sleep(0.6),
                                        vm.shot('12_menu_open'),
                                        vm.move_to(100, 66), vm.click(),
                                        time.sleep(0.6),
